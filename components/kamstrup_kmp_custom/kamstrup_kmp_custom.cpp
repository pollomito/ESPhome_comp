#include "kamstrup_kmp_custom.h"

#include "esphome/core/log.h"

namespace esphome {
namespace kamstrup_kmp_custom {

static const char *const TAG = "kamstrup_kmp_custom";

void KamstrupKMPComponent::dump_config() {
  ESP_LOGCONFIG(TAG, "kamstrup_kmp_custom:");
  if (this->is_failed()) {
...
void KamstrupKMPComponent::read_command_(uint16_t command) {
  ...
  while (timeout > 0) {
    if (this->available()) {
      data = this->read();
      if (data > -1) {
        if ((data == 0x40) && ((buffer_len == 0) || (buffer_len == 9)))  // 0 if no echo, 9 if echo (tx mess len = 9)
		{  // start of message
		  startfound = 1;
          buffer_len = 0;
        }
